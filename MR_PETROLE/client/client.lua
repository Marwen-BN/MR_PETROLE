...
local blips = {
    { name = 'Petrol', sprite = 1078668923, x =484.9, y = 599.18, z = 111.4},
...
Citizen.CreateThread(function()
    while true do
        ...
        if (Vdist(pos.x, pos.y, pos.z, Config.Zonas['init'].x, Config.Zonas['init'].y, Config.Zonas['init'].z) < 3.0) then
            if Mina1 == false then
                DrawTxt(Language.translate[Config.lang]['press'], 0.3, 0.95, 0.4, 0.4, true, 241, 15, 11, 255, false)
		...
		if PrimeraMina == true then
            ...
            if (Vdist(pos.x, pos.y, pos.z, Config.Zonas['Miner1'].x, Config.Zonas['Miner1'].y, Config.Zonas['Miner1'].z) < 3.0) then
                ...
                if IsControlJustPressed(0, 0xC7B5340A) then
					...
					Wait(10000)
					RSGCore.Functions.Notify( Language.translate[Config.lang]['carry'], 'success', 6000)
					Wait(5000)
					animacion3(Arbol1)
					RemoveBlip(blip)
            ...
            if (Vdist(pos.x, pos.y, pos.z, Config.Zonas['Vehicle'].x, Config.Zonas['Vehicle'].y, Config.Zonas['Vehicle'].z) < 3.0) then
                DrawTxt(Language.translate[Config.lang]['pressc'], 0.3, 0.95, 0.4, 0.4, true, 241, 15, 11, 255, false)
                if IsControlJustPressed(0, 0xC7B5340A) then
                    animacion2()
                    RSGCore.Functions.Notify( Language.translate[Config.lang]['goto'], 'success', 6000)
                    RemoveBlip(blip5)
        ...
        elseif FinMina == true then
            if (Vdist(pos.x, pos.y, pos.z, Config.Zonas['Vehicle'].x, Config.Zonas['Vehicle'].y, Config.Zonas['Vehicle'].z) < 3.0) then
                ...
                if IsControlJustPressed(0, 0xC7B5340A) then
                    animacion2()
					Wait(10000)
                    RSGCore.Functions.Notify( Language.translate[Config.lang]['noveh'], 'success', 6000)
                    RemoveBlip(blip5)
        ...
        elseif Entrega == true then
            ...
            if (Vdist(pos.x, pos.y, pos.z, Config.Zonas['Entrega'].x, Config.Zonas['Entrega'].y, Config.Zonas['Entrega'].z) < 3.0) then
                DrawTxt(Language.translate[Config.lang]['pressf'], 0.3, 0.95, 0.4, 0.4, true, 241, 15, 11, 255, false)
                if IsControlJustPressed(0, 0xC7B5340A) then
                    if IsPedInAnyVehicle(PlayerPedId(), true) then
                        DeleteVehicle(GetVehiclePedIsIn(PlayerPedId()))
						RSGCore.Functions.Notify( Language.translate[Config.lang]['completejob'], 'success', 6000)
						RemoveBlip(blip6)
...
AddEventHandler('MR_Petrol:TIME', function()
	...
	Citizen.CreateThread(function()
		while Entrega do
            ...
            if timer < 1 then
                RSGCore.Functions.Notify( Language.translate[Config.lang]['lose'], 'success', 6000)
